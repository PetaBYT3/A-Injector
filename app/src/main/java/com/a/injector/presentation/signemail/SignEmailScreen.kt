...
@Composable
fun SignEmailScreenRoot(
    navBackStack: NavBackStack<NavKey>,
    viewModel: SignEmailViewModel = koinViewModel()
) {
...
@Composable
private fun SignEmailScreen(
    navBackStack: NavBackStack<NavKey>,
    state: SignEmailState,
    onAction: (SignEmailAction) -> Unit,
    snackBarHostState: SnackbarHostState
) {
    Scaffold(
        topBar = {
        ...
        if (!state.isVerifyingOtp) {
            ...
            item("emailTextField") {
                CustomTextField(
                    ...
                    onValueChange = { onAction(SignEmailAction.EmailTextField(it)) }
                )
            }
            spacer()
            item("sendButton") {
                Box(
                    ...
                    CustomButton(
                        ...
                        onClick = { onAction(SignEmailAction.SendOtpButton) },
        ...
        item("otpTextField") {
            CustomTextField(
                modifier = Modifier
                    .animateItem(),
                label = "OTP",
                value = state.otpTextField,
                onValueChange = { onAction(SignEmailAction.OtpTextField(it)) }
            )
        }
        spacer()
        item("sendButton") {
            Box(
                ...
                CustomButton(
                    modifier = Modifier
                        .height(ButtonDefaults.MediumContainerHeight),
                    onClick = { onAction(SignEmailAction.VerifyOtpButton) },
                    text = "Verify OTP",
                    isLoading = state.isVerifyOtpButtonLoading
                )
            }
        }
        return@LazyColumn
